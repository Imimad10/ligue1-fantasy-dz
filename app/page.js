...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (playersRes.error) {
        console.error("Erreur chargement joueurs:", playersRes.error.message)
      } else if (playersRes.data) {
        setPlayers(playersRes.data)
      }
      if (teamsRes.error) {
        console.error("Erreur chargement équipes:", teamsRes.error.message)
  ...
  const saveTeam = async () => {
    ...
    // Récupérer la gameweek courante
    ...
    if (fantasyTeamId) {
      // Mettre à jour le nom et budget
    ...
    if (pErr) {
      ...
      setSaveMsg({ type: 'success', text: 'Équipe mise à jour avec succès ! 🎉' })
  ...
  const autoSelect = () => {
    const maxTenths = 1000 // 100.0M max budget (100 * 10)
    ...
    // Trier par prix décroissant pour chaque poste
    ...
    // Sélectionner les meilleurs joueurs tout en réservant le budget minimal pour les slots restants
    for (const pos of ['FWD', 'MID', 'DEF', 'GK']) {
      ...
      for (const player of byPos[pos]) {
        ...
        // Calculer le coût minimal garanti pour les emplacements restants non remplis
      ...
      // Fallback si la boucle n'a pas pu compléter le poste
    ...
    // Boucle d'ajustement de sécurité : si le budget dépasse 100.0M, échanger les joueurs les plus chers par des options moins chères
